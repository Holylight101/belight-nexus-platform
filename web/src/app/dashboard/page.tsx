...
export default function DashboardPage() {
  return (
    <div className="p-8">
      <div className="mb-10">
        <h1 className="text-3xl font-bold text-white">Welcome to BeLight Nexus AI</h1>
        <p className="text-gray-400 mt-2">Your intelligent business platform. Three agents. One mission.</p>
      </div>
          ...
          <Link key={agent.name} href={agent.href}>
            <div className={`bg-gradient-to-br ${agent.color} p-6 rounded-2xl cursor-pointer hover:scale-105 transition-transform`}>
              ...
              <div className="bg-white/20 rounded-lg px-3 py-1 inline-block">
                <span className="text-white text-xs font-medium">{agent.stat}</span>
              </div>
            </div>
          </Link>
        ))}
